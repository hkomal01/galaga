...
class RenderingSystem:
    ...
    def renderBullets(self, shipBullet_entity, alienBullet_entity):
        ...
        for mov in shipBullet_entity.movement:
            pygame.draw.rect(game.SCREEN, "red", pygame.Rect(mov.position.x, 
                                                        mov.position.y, 5, 15))
    ...
    def renderHud(self, ship_entity):
        ...
        for i in range(min(health, 10)):
            hImage = pygame.transform.scale(ship_entity.sprite[0].image, 
                                (ship_entity.sprite[0].image.get_width() * 0.5, 
                                  ship_entity.sprite[0].image.get_height()*0.5))
    ...
    def renderStar(self, start_entity):
        ...
        """
        color = (255, 255, 255)
        for (starSize, starMovement) in zip(start_entity.size, 
                                                start_entity.movement):
